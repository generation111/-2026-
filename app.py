...
st.markdown("""
    ...
    .section-head { font-size: 17px; font-weight: bold; color: #007bff; background-color: #f0f7ff; padding: 6px 12px; border-radius: 5px; margin: 15px 0 10px 0; border-left: 5px solid #007bff; }
    .stSelectbox label, .stTextInput label, .stDateInput label { font-weight: 700 !important; color: #34495e !important; }
    .stButton button { width: 100%; background: linear-gradient(135deg, #007bff 0%, #0056b3 100%); color: white; height: 50px; font-size: 18px !important; font-weight: bold; border-radius: 10px; margin-top: 20px; border: none; }
    </style>
...
def get_g_client():
    try:
        # 從 Secrets 讀取 TOML 格式的金鑰
        creds_info = st.secrets["gcp_service_account"]
        scope = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive"]
        creds = Credentials.from_service_account_info(creds_info, scopes=scope)
        return gspread.authorize(creds)
    except Exception as e:
        st.error(f"🔑 金鑰讀取失敗，請檢查 Streamlit Secrets 設定。")
        return None
...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(["📝 快速錄入", "📊 年度明細清單", "🔔 預購追蹤"])
    ...
    with tab1:
        with st.form("main_form", clear_on_submit=True):
            ...
            st.markdown('<div class="section-head">📅 行政資訊</div>', unsafe_allow_html=True)
            ...
            st.markdown('<div class="section-head">📦 產品項目</div>', unsafe_allow_html=True)
